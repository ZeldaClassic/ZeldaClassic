...
void scc_insert(char* buf, size_t& msgptr,byte cc,size_t limit = -1)
{
	switch(cc) //special scc inserts
	{
		case MSGC_MENUCHOICE:
			...
			break;
		case MSGC_DRAWTILE:
			buf[msgptr++] = '['; if(msgptr >= limit) break;
			buf[msgptr++] = ']';                     break;
		case MSGC_NAME:
			buf[msgptr++] = '('; if(msgptr >= limit) break;
			buf[msgptr++] = 'N'; if(msgptr >= limit) break;
			buf[msgptr++] = 'a'; if(msgptr >= limit) break;
			buf[msgptr++] = 'm'; if(msgptr >= limit) break;
			buf[msgptr++] = 'e'; if(msgptr >= limit) break;
			buf[msgptr++] = ')';                     break;
	}
}
