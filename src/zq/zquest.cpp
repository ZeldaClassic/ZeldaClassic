...
void draw_autocombo(int32_t pos, bool rclick, bool pressframe)
{
	...
	if (ca.valid())
	{
		switch (ca.getType())
		{
			...
			case AUTOCOMBO_Z1:
			{
				AutoPattern::flatmtn ap(ca.getType(), CurrentLayer, scr, pos, &ca);
				if (rclick)
					ap.erase(scr, pos);
				else
					ap.execute(scr, pos);
...
bool handle_slot(script_slot_data& slotdata, int indx, script_data** scriptdata)
{
	if(slotdata.hasScriptData())
	{
		...
		parse_script_string(scriptdata[indx],scriptstr.c_str(), false);
...
void Z_eventlog([[maybe_unused]] const char *format,...)
{
}
...
int32_t main(int32_t argc,char **argv)
{
	...
	if (package_arg > 0)
	{
		...
		string error = package_create(input_filename, package_name);
